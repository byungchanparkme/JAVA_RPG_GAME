...
public class WarriorAttackSkill extends Skill {
    private double attackDamagePercent;

    public WarriorAttackSkill(String name, int mpConsumption) {
        super(name, mpConsumption);
        this.attackDamagePercent = 1.65;
    }
    ...
    public void powerStrike(Player player, Monster monster) {
        double damage = (player.getAttackPower() * attackDamagePercent - (monster.getShieldPower() / 2.0) <= 0) ? 0 : player.getAttackPower() * attackDamagePercent - (monster.getShieldPower() / 2.0);
